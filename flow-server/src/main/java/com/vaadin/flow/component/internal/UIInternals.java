...
public class UIInternals implements Serializable {
    ...
    public List<JavaScriptInvocation> dumpPendingJavaScriptInvocations() {
        ...
        List<JavaScriptInvocation> readyToSend = getPendingJavaScriptInvocations()
                .stream()
                .filter(invocation -> invocation.getOwner().isEmpty() || invocation.getOwner().get().isVisible())
                .collect(Collectors.toList());

        pendingJsInvocations = getPendingJavaScriptInvocations()
                .stream()
                .filter(invocation -> invocation.getOwner().isPresent() && !invocation.getOwner().get().isVisible())
